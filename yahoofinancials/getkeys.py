#!/usr/bin/env python3
# retrieve keys and store to file
#
# from https://github.com/ranaroussi
#
...
KEY_URL = 'https://raw.githubusercontent.com/ranaroussi/yfinance/main/yfinance/scrapers/yahoo-keys.txt'
KEYFILE = '.keys.txt'
KEYPATH = f"{sys.path[0]}/{KEYFILE}"

try:
    with open(KEYPATH, 'r') as f:
        # splitlines avoids needing to strip every line
        existkeys = f.read().splitlines()
...
if resp.status_code != 200:
    print(f"http request failed ({resp.status_code})", file=sys.stderr)
    sys.exit(1)
dlkeys = resp.text.splitlines()

dupes = [n for n, k in enumerate(dlkeys) if k in existkeys]
if dupes:
    if len(dupes) == len(dlkeys):
        print("No new keys", file=sys.stderr)
        sys.exit(0)
    else:
        print(f"Found {len(dupes)}/{len(dlkeys)} already known keys: {dupes}", file=sys.stderr)

with open(KEYFILE, 'a') as f:
    [f.write(k + '\n') for k in dlkeys if k not in existkeys]
    # if last was a dupe, make sure it is also last
    if dlkeys and dlkeys[-1] in existkeys and dlkeys[-1] != existkeys[-1]:
        f.write(dlkeys[-1] + '\n')
...
# where do those hex strings come from?  probably deobfuscation of main.js
