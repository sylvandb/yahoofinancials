...
import re
from sys import stderr as errout
from time import sleep, strftime, strptime

from .exceptions import ManagedException, ParseException, URLOpenException
from .fetchurl import READ_TRIES, LOG_FAILURES, _debug, _fetch_url, _time_report, _trace
from .decrypt import decrypt as _decrypt
...
class YahooFinanceETL(object):
    ...
    def format_date(in_date):
        if isinstance(in_date, str):
            form_date = int(calendar.timegm(strptime(in_date, '%Y-%m-%d')))
        else:
    ...
    def _parse_finance(self, url, content):
        ...
        try:
            ...
            self._cache[url] = _decrypt(datastore, data_obj, soup)
        except Exception as e:
            # no point in trying this over and over again so remember the parse failure
            self._cache[url] = ParseException(
                    "Failed, missing stores in %s bytes from '%s': %s" % (len(str(soup)), url, e))
            self._cache[url].data = str(soup)
            raise self._cache[url]
    ...
    def _fetch_finance(self, url):
        ...
        for tries in range(READ_TRIES):
            ...
            if tries:
                ...
                sleep((2 ** tries) / 2 + randrange(1, 5))
            rescode, response_content = _fetch_url(url, self.activitycb)
            _trace('rescode=%s' % rescode)
            if rescode == 200:
                self._parse_finance(url, response_content)
                break
            print("Fail try %d, code %d from %s" % (tries + 1, rescode, url), file=errout)
        else:
            print("Failed, code %d from %s" % (rescode, url), file=errout)
    ...
    def _get_api_data(self, api_url):
        ...
        for tries in range(READ_TRIES):
            ...
            rescode, res_content = _fetch_url(api_url, self.activitycb)
            try:
                if rescode == 200:
    ...
    def _create_dict_ent(self, up_ticker, statement_type, tech_type, report_name, hist_obj):
        ...
        if statement_type == 'history':
            ...
            try:
                re_data = _time_report(self._scrape_finance, yahoo_url)["QuoteSummaryStore"]
            except KeyError:
                re_data = None
            try:
                if tech_type:
    ...
    def get_stock_data(self, statement_type='income', tech_type='', report_name='', hist_obj={}):
        ...
        for tick in self.ticker:
            exc = None
            try:
                dict_ent = _time_report(self._create_dict_ent, tick, statement_type, tech_type, report_name, hist_obj)
                data.update(dict_ent)
            except URLOpenException as e:
                print("Warning! Ticker: %s: %s" % (tick, e), file=errout)
                exc = e
            except ParseException as e:
                print("Warning! Ticker: %s: %s" % (tick, e), file=errout)
                exc = e
                ...
                if exc:
                    print("The process is still running...", file=errout)
                    if LOG_FAILURES:
